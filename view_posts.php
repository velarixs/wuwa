...
if(isset($_POST['add_comment'])){
   ...
   $user_first_name = filter_var($user_first_name, FILTER_SANITIZE_STRING);
   $comment = $_POST['comment'];
   $comment = filter_var($comment, FILTER_SANITIZE_STRING);

   $verify_comment = $conn->prepare("SELECT * FROM `comments` WHERE post_id = ? AND user_id = ? AND user_first_name = ? AND comment = ?");
   $verify_comment->execute([$get_id, $user_id, $user_first_name, $comment]);

   if($verify_comment->rowCount() > 0){
      $message[] = 'comment already added!';
   }else{
      $insert_comment = $conn->prepare("INSERT INTO `comments`(post_id, user_id, user_first_name, comment) VALUES(?,?,?,?)");
      $insert_comment->execute([$get_id, $user_id, $user_first_name, $comment]);
...
<section class="comments-container">
   ...
   <form action="" method="post" class="add-comment">
      <input type="hidden" name="user_id" value="<?= $fetch_user_id['user_id']; ?>">
      <input type="hidden" name="user_first_name" value="<?= $fetch_profile['first_name']; ?>">
      <p class="user"><i class="fas fa-user"></i><a href="update.php"><?= $fetch_profile['first_name']; ?></a></p>
